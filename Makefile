...
provider:: generate_schema
	...
	cd provider && go install -ldflags "-X github.com/pulumi/pulumi-datadog/provider/v2/pkg/version.Version=${VERSION}" ${PROJECT}/provider/v2/cmd/${PROVIDER}

generate_schema:: tfgen
	$(TFGEN) schema --out ./provider/cmd/${PROVIDER}

tfgen::
	cd provider && go install -ldflags "-X github.com/pulumi/pulumi-datadog/provider/v2/pkg/version.Version=${VERSION}" ${PROJECT}/provider/v2/cmd/${TFGEN}
