...
provider:: generate_schema
	...
	cd provider && go install -ldflags "-X github.com/pulumi/pulumi-datadog/provider/pkg/version.Version=${VERSION}" ${PROJECT}/provider/cmd/${PROVIDER}

generate_schema:: tfgen
	$(TFGEN) schema --out ./provider/cmd/${PROVIDER}

tfgen::
	cd provider && go install -ldflags "-X github.com/pulumi/pulumi-datadog/provider/pkg/version.Version=${VERSION}" ${PROJECT}/provider/cmd/${TFGEN}
